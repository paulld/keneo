...
if (isset($_SESSION['mot_de_passe']) AND $_SESSION['mot_de_passe'] == $_SESSION['pass'])
{
	...
	{
		$id = $_POST['IDinact'];
		$bdd->query("UPDATE rob_nature1 SET actif=0 WHERE ID='$id'");
	}
	else
	{
		...
		{
			$id = $_POST['IDact'];
			$bdd->query("UPDATE rob_nature1 SET actif=1 WHERE ID='$id'");
		}
		else
		{
			if (isset($_POST['newcode']))
			{
				...
				$checkcode = $bdd->query("SELECT code FROM rob_nature1 WHERE code='$code'");
				$codepris = $checkcode->rowCount();
				$checkcode->closeCursor();
				if ($codepris != 0)
				{
					echo 'Cette nature 1 existe d&eacute;j&agrave';
				}
				else
				{
					$desc = $_POST['newdesc'];
					$desc = str_replace("'","\'",$desc);
					$bdd->query("INSERT INTO rob_nature1 VALUES('', '$code', '$desc', 1)");
				}
			}
			else
			{
				if (isset($_POST['modID']))
				{
					...
					$desc = $_POST['moddesc'];
					$desc = str_replace("'","\'",$desc);
					$bdd->query("UPDATE rob_nature1 SET Description='$desc', code='$code' WHERE ID='$modID'");
				}
			}
		}
	}
	...
	<div id="navigationMap">
		...
		<li><a class="typ" href="#"><span>Nature 1</span></a></li></ul>
	</div>
	<div id="clearl"></div>
